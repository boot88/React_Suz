...
const AVATAR_STORAGE_KEY = 'employeeChatAvatars';
...
const saveAvatar = (username, dataUrl) => {
...
const EmployeeChat = () => {
  ...
  useEffect(() => {
    const avatar = readAvatars()[user?.username] || '';
    setAvatarPreview(avatar);

    const fullName = user?.name || user?.username || '';
    setHeaderText(`Здравствуйте, ${fullName}!`);
    const timeout = setTimeout(() => setHeaderText(fullName), 2000);
  ...
  const availableEmployees = useMemo(() => {
    ...
    return sourceEmployees
      ...
      .map((item) => {
        ...
        return {
          email: item.login,
          fullName: item.full_name || item.login,
          phone: item.phone || item.internal_phone || '',
  ...
  useEffect(() => {
    ...
    const isManagerSelected = selectedEmail === MANAGER_CREDENTIALS.username;
    const existsInDirectory = directoryEmployees.some((item) => item.login === selectedEmail);

    if (!existsInDirectory && !isManagerSelected) {
      setSelectedEmail('');
    }
  ...
  const handleAvatarUpload = (event) => {
    ...
    if (!ALLOWED_AVATAR_TYPES.includes(file.type)) {
      window.alert('Допустимы только JPG, PNG или WEBP.');
      return;
    }

    if (file.size > 2 * 1024 * 1024) {
      window.alert('Максимальный размер аватара — 2 МБ.');
      return;
    }
      ...
      <aside className="employee-chat-sidebar">
        ...
        <div className="employee-profile">
          <label className="avatar-upload">
            ...
            {avatarPreview ? (
              <img src={avatarPreview} alt="Аватар сотрудника" className="avatar-image" />
            ) : (
          ...
          {availableEmployees.map((employee) => {
            ...
            return (
              ...
              >
                ...
                <div className="employee-card-tooltip">
                  <div><strong>{employee.fullName || employee.email}</strong></div>
                  <div>Почта: {employee.email}</div>
                  <div>Телефон: {employee.phone || '—'}</div>
                  <div>Отдел: {employee.department || '—'}</div>
                  <div>Кабинет: {employee.room || '—'}</div>
                </div>
              </button>
            );
          })}

...
const TEMPLATE_MESSAGES = ['✅ Принято в работу', '🔧 Проверяю сейчас', '👍 Спасибо, получил', '🕒 Вернусь с ответом в течение 10 минут', '📌 Пожалуйста, уточните детали проблемы', '📨 Передал информацию ответственному сотруднику'];
...
const EmployeeChat = () => {
  ...
  const currentConversationId = useMemo(() => (
    selectedEmail ? getConversationId(user.username, selectedEmail) : null
  ), [selectedEmail, user.username]);
    ...
    return sourceEmployees
      ...
      .map((item) => {
        ...
        return {
          ...
          profile: item,
          displayName: item.full_name || item.login,
          room: item.room || '—',
          isManagerAccount: item.login.toLowerCase() === MANAGER_CREDENTIALS.username.toLowerCase()
        };
      })
      .sort((a, b) => {
        if (a.isManagerAccount !== b.isManagerAccount) return a.isManagerAccount ? -1 : 1;
    ...
    <div className="employee-chat-layout">
        ...
        <div className="employee-chat-header">
          <div className="employee-avatar-wrap">
            <button
              ...
              title={avatarUrl ? 'Открыть аватар' : 'Добавить аватар'}
            ...
            <input ref={avatarInputRef} id="avatar-upload-input" type="file" accept="image/png,image/jpeg,image/webp" onChange={handleAvatarUpload} hidden />
            ...
            <div className="employee-header-meta">
              ...
              <button type="button" className="avatar-edit-btn" onClick={() => avatarInputRef.current?.click()}>Изменить фото</button>
            </div>
          </div>
        ...
        <div className="employee-chat-list">
          {availableEmployees.map((employee) => {
            ...
            return (
              <button
                key={employee.email}
                className={`employee-chat-user ${selectedEmail === employee.email ? 'active' : ''} ${employee.isManagerAccount ? 'manager-chat' : ''}`}
                onClick={() => setSelectedEmail(employee.email)}
              >
                <span className={`status-dot ${isOnline ? 'online' : 'offline'}`} />
                <div className="employee-chat-user-main">
                  <span className="employee-chat-user-email">{employee.email}</span>
                  <span className="employee-chat-user-details">{employee.displayName} · каб. {employee.room}</span>
                </div>
                <span className="employee-chat-user-status">{employee.isManagerAccount ? 'manager' : (isOnline ? 'online' : 'offline')}</span>
      ...
      <section className="employee-chat-main">
        ...
        {showAvatarPreview && avatarUrl && (
          <div className="avatar-modal" onClick={() => setShowAvatarPreview(false)}>
            <div className="avatar-modal-content" onClick={(e) => e.stopPropagation()}>
              <img src={avatarUrl} alt="avatar-full" />
              <button type="button" onClick={() => setShowAvatarPreview(false)}>Закрыть</button>
            </div>
          </div>
        )}

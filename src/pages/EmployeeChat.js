...
const DEFAULT_TEMPLATES = [
  'Коллеги, нужна помощь по заявке.',
  'Подтвердите, пожалуйста, выполнение задачи.',
...
const processAvatarImage = (file) => new Promise((resolve, reject) => {
  ...
  reader.onload = () => {
    ...
    img.onload = () => {
      ...
      const targetSize = Math.min(512, side);
...
const EmployeeChat = () => {
  ...
  const [employeeForm, setEmployeeForm] = useState({ id: null, login: '', password: '', full_name: '', department: '', phone: '', room: '' });
  ...
  useEffect(() => {
    ...
    setHeaderName(`Здравствуйте, ${baseDisplayName}!`);
  ...
  const chatCandidates = useMemo(() => {
    ...
    return sourceEmployees
      ...
      .map((item) => {
        ...
        return { email: item.login, isOnline: Boolean(presence?.isOnline), lastSeen: presence?.lastSeen || null, profile: item };
      })
      .sort((a, b) => {
  ...
  const unreadByEmail = useMemo(() => {
    ...
    chatCandidates.forEach((employee) => {
      ...
      map[employee.email] = (threads[conversationId] || []).filter((message) => message.sender !== user.username && new Date(message.createdAt).getTime() > lastReadAt).length;
    });
    return map;
  }, [chatCandidates, readState, threads, user.username]);

  const pinnedMessages = useMemo(() => currentMessages.filter((item) => item.pinned), [currentMessages]);
  ...
  useEffect(() => {
    ...
    const latestIncoming = (threads[currentConversationId] || [])
      ...
      .reduce((latest, message) => !latest || new Date(message.createdAt).getTime() > new Date(latest).getTime() ? message.createdAt : latest, null);
  ...
  const handleAvatarUpload = async (event) => {
    ...
    if (file.size > 4 * 1024 * 1024) {
      window.alert('Рекомендуем загружать фото до 4MB.');
      return;
    }
    ...
    try {
      ...
      localStorage.setItem(getAvatarKey(user.username), processed);
    } catch (error) {
  ...
  const updateMessage = async (targetConversationId, updater) => {
    const nextMessages = (threads[targetConversationId] || []).map((item) => updater(item));
  ...
  const editMessage = async (messageId, targetConversationId = currentConversationId) => {
    ...
    try {
      await updateMessage(targetConversationId, (item) => (
        item.id !== messageId ? item : { ...item, text: nextText.trim(), editedAt: new Date().toISOString() }
      ));
    } catch (error) {
  ...
  const togglePin = async (messageId) => {
    if (!currentConversationId) return;
    try {
      await updateMessage(currentConversationId, (item) => (
        item.id !== messageId ? item : { ...item, pinned: !item.pinned }
      ));
    } catch (error) {
        ...
        <div className="employee-chat-header">
          <div className="employee-avatar-wrap">
            ...
            <div className="employee-header-meta">
              ...
              <p className={isGreetingActive ? 'greeting-active' : ''}>{headerName}</p>
              <small className="avatar-tip">Для лучшего качества используйте квадратное фото.</small>
            </div>
          </div>
        </div>
        ...
        {!selectedEmail ? (
          ...
          <>
            <div className="conversation-header">
              <span>Диалог с {selectedEmail}</span>
              {draft.trim() && <em className="typing-indicator">Вы печатаете...</em>}
            </div>

            {pinnedMessages.length > 0 && (
              <div className="pinned-wrap">
                {pinnedMessages.map((message) => (
                  <button key={message.id} type="button" className="pinned-item" onClick={() => setReplyTo(message)}>
            ...
            <div className="messages-wrap">
              ...
              {currentMessages.map((message, index) => {
                ...
                const hasRead = isMine && currentMessages.slice(index + 1).some((item) => item.sender !== user.username);
                    ...
                    <div className="message-bubble">
                      ...
                      {message.replyTo && (
                        <div className="reply-preview">
                          Ответ на: <strong>{message.replyTo.sender}</strong> — {message.replyTo.text}
                        </div>
                      )}
                      ...
                      <div className="reaction-row">
                        {['👍', '✅', '🔧'].map((emoji) => {
                          const count = (message.reactions?.[emoji] || []).length;
                          const active = (message.reactions?.[emoji] || []).includes(user.username);
                          return (
                            <button key={emoji} className={`reaction-btn ${active ? 'active' : ''}`} onClick={() => toggleReaction(message.id, emoji)}>
                              {emoji} {count > 0 ? count : ''}
                            </button>
                          );
                        })}
                      ...
                      <div className="message-status">{isMine ? (hasRead ? 'прочитано' : 'доставлено') : ''}</div>

                      {canEdit && (
                        <div className="message-controls">
                          <button onClick={() => setReplyTo(message)}>Ответить</button>
                          <button onClick={() => togglePin(message.id)}>{message.pinned ? 'Открепить' : 'Закрепить'}</button>
            ...
            {replyTo && (
              <div className="reply-compose">
                Ответ на: <strong>{replyTo.sender}</strong> — {replyTo.text}
                <button type="button" onClick={() => setReplyTo(null)}>×</button>
              </div>
            )}

            <form className="message-form" onSubmit={handleSend}>
              <input placeholder="Введите сообщение..." value={draft} onChange={(e) => setDraft(e.target.value)} maxLength={2000} />
        ...
        {isManager && (
          <div className="manager-panels">
            <section className="manager-panel">
              ...
              <form className="manager-form" onSubmit={async (e) => {
                e.preventDefault();
                  ...
                  <div className="manager-list-item" key={employee.id}>
                    ...
                    <div className="manager-list-actions">
                      ...
                      <button onClick={async () => {
                        ...
                        const response = await fetch(`${API_BASE_URL}/auth/employees/${employee.id}`, { method: 'DELETE' });
                        const data = await response.json().catch(() => ({}));
                        if (!response.ok) {
            ...
            <section className="manager-panel">
              ...
              <div className="threads-grid">
                <div className="threads-list">
                  {Object.keys(threads).sort().map((threadId) => {

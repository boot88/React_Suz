...
const TEMPLATE_MESSAGES = [
  ...
  '🔧 Проверяю сейчас',
  '👍 Спасибо, получил',
  '🕒 Вернусь с ответом в течение 10 минут',
  '📌 Добавил в приоритет',
...
const EmployeeChat = () => {
  ...
  const currentMessages = useMemo(() => (currentConversationId ? (threads[currentConversationId] || []) : []), [currentConversationId, threads]);
  const selectedThreadMessages = useMemo(() => (selectedThreadId ? (threads[selectedThreadId] || []) : []), [selectedThreadId, threads]);
  ...
  const chatCandidates = useMemo(() => {
    ...
    return sourceEmployees
      ...
      .map((item) => {
        ...
        const isManagerAccount = item.login.toLowerCase() === MANAGER_CREDENTIALS.username.toLowerCase();
        return {
          email: item.login,
  ...
  return (
    <div className="employee-chat-layout">
      <aside className="employee-chat-sidebar">
        <div className="employee-chat-header">
          <div className="employee-avatar-wrap">
            <button type="button" className="employee-avatar-upload" title="Открыть аватар" onClick={() => avatarUrl && setIsAvatarOpen(true)}>
          ...
          {availableEmployees.map((employee) => {
            ...
            return (
              ...
              >
                ...
                <span className="employee-chat-user-meta">{employee.profile?.full_name || 'Без ФИО'} · каб. {employee.profile?.room || '—'}</span>
                {employee.isManagerAccount && <span className="manager-badge">Менеджер</span>}
      ...
      {isAvatarOpen && avatarUrl && (
        <div className="avatar-modal" onClick={() => setIsAvatarOpen(false)}>
          <div className="avatar-modal-content" onClick={(e) => e.stopPropagation()}>
            <img src={avatarUrl} alt="avatar full" />
            <button type="button" onClick={() => setIsAvatarOpen(false)}>Закрыть</button>
          </div>
        </div>
      )}
